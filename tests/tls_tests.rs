...
fn server_tls_requires_cert_and_key_when_enabled() {
    ...
    assert!(
        error.to_string().contains("certificate chain")
...
fn client_tls_without_client_cert_still_builds_config() {
    let tls = TlsConfig::builder()
        ...
        .build();
    let config = rustls::build_client_config(&tls)
        .expect("client config should build without client auth cert");
...
fn use_system_roots_default_is_true() {
    ...
    assert!(
        ...
        "use_system_roots should default to true"
    );
}
